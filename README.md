...
- [**Deployment tracker (current cycle)**](docs/todos/deployment.md) — everything pending to ship the 2026-08-07/08 work (flood ontology, author-context geocoding, Ciudad situation PoC): kg commits + listener restart, gp3 whitelist redeploy, apify author-enrichment warm-up, geocoder KB repairs + multi-context training, cc ship decision, env/tunnel hygiene.
- [**Author-context geocoding — ship it correctly**](docs/todos/author_context_geocoding_rollout.md) — the author's declared location as geocoder `context_group` 2. The envelope dropped `_author_geo` until `bd5694f`, so the feature had **never actually run**; re-measured on the full fb/x corpus it shows **0 improvements and 1 degradation** (p7→p2 on a Tijuana account posting about CDMX), now neutralized by a degradation guard. Open: geocoder multi-context CER retraining, the `gp3` `location_author` whitelist deploy, and re-measurement on **local sources** — where anchor-less posts should finally make the feature visible.
- [**Productionization: streaming KG → live kgdb**](docs/todos/productionization_streaming_kg.md) — go-live checklist for running the listener continuously against **live kgdb**. Decisions locked: **production-only** writer (kgdb is shared append-only ground truth — no staging/prod double-write; dev kgdb is the test target), **single worker first** (until [canonical reconciliation](docs/todos/canonical_reconciliation.md) lands), post-`gp3` **firehose MVP** producer. **Phase 1 (live-kgdb schema/seeds) done** — all migrations + seeds verified applied on live kgdb (2026-08-04). Remaining phased work: prod config/secrets, flip the `gp3`→kg-queue producer (`KG_QUEUE`), k8s deploy, observability, canary cutover.
...
- [New ontology class: missing person / Amber Alert](docs/todos/missing_person_amber_class.md) — Amber Alert posts are clean structured events with no matching class; the 2026-08-08 CDMX social batch missed the same alert 3 times. New `missing_person` class + keywords, likely extraction-only (no geo-linking) at first.
...
- [Extract a list of locations, not a single one](docs/todos/location_level_list_extraction.md) — model `locations: List[Location]` so multi-street/venue events (e.g. "calles San Juan del Río y Amealco") geocode to several level-6/7 places, letting two records that share a street merge deterministically. Fixes the coarse-precision under-merge (the El Marqués case). **Urgency raised 2026-08-30 (CDMX marathon):** the Parser was silently nulling list-valued `location`s, so multi-site events lost geo entirely; the shipped interim coalesce keeps the common prefix and preserves the list as `_locations` — the linker/writer multi-location plumbing (changes 3–5) can consume it directly.
...
- [Structured cartelera ingestion (venue/ticketing listings → kg queue)](docs/todos/structured_cartelera_ingestion.md) — small-venue coverage gap (Foro Indie Rocks: 2 events ever in kgdb despite near-nightly gigs): scrape structured listings (Ticketmaster Discovery API + venue sites) and publish **pre-extracted** `paid_mass_event` records to the kg queue — the listener skips matching/LLM extraction and goes straight to normalize → geocode → link → persist, so structured records merge with (and date-anchor) the press-derived canonicals.
...
- [Extraction: standardize extracted-date timezones](docs/todos/extraction_date_timezone_normalization.md) — **fix implemented (2026-08-06):** `America/Mexico_City` pinned as the parser default (`base.py` `local_tz`), and `_normalize_date_timezones` re-anchors naive/UTC-stamped extracted datetimes to the record's `timezone` (else the default) — previously the UTC Docker listener stamped `+00:00` on Mexico-local times (event 483: "10:00 horas" stored as 10:00 UTC). **Remaining:** repair of old rows' `entities.metadata.date_range` `+00:00` stamps (the user-facing `event_properties.event_date_*` columns were already backfilled corrected).
- [Extraction: day↔month swap in ISO dates](docs/todos/extraction_date_daymonth_swap.md) — the extraction LLM sometimes emits `date_range.start`/`end` with day and month transposed (`"11 de junio"` → `2026-11-06`), forking date-shadow twins; root-caused to the model output (the schema parser is correct). Prompt fix + a deterministic mention→ISO reconciliation guard. Surfaced by the reconciliation dry-run.
- [Merge: narrow sub-event window overwrites an exact multi-day span](docs/todos/merge_narrow_window_overwrites_span.md) — `_apply_best_window` ranks by narrowness, conflating duration with uncertainty: a festival's exact «del 6 al 16» range loses to any later single-day *función* mention (33 live rows narrowed; two currently-running festivals invisible to the consumer's day-window queries). Direction: infer instance-vs-umbrella layer at merge time and pick the **best observed container** window for umbrellas; repair pass recomputes from the intact `_source_windows` ledger.
- [Merge: best-window lets a precise start-only mention veto end dates](docs/todos/merge_best_window_blocks_end_dates.md) — sibling failure mode of the same rule: a `precision_days: 0` start-only announcement is unbeatable, so later end-bearing windows (open-ended closures, plantones) never land. Direction: pick start and end independently.
- [Extraction: `precision_days` usually null (80%)](docs/todos/extraction_precision_days_missing.md) — 80% of extracted events omit `precision_days`, so vague month/year mentions look as precise as a single day. Merge-side ranking now mitigated (`_effective_precision_days`: width-fallback, `null`≠exact); the real fix is making extraction always emit `precision_days` (and an `end` for periods).
...
- [Skip geocoding foreign-location events (mark out-of-scope)](docs/todos/geocode_foreign_out_of_scope.md) — extracted events located outside Mexico get geocoded anyway and land in kgdb looking like badly-geocoded Mexican events; a ~5-line client-side rule (`country != Mexico` → `_geo_source: out_of_scope`, no geocoder call).
- [Geocoder mismatches: homonym streets, discarded anchors, KB gaps](docs/todos/geocoder_mismatches.md) — 9 verified wrong-homonym street matches from the 2026-08-30 CDMX marathon corpus (Oaxaca→Contreras, Insurgentes→Iztapalapa, …), each replayed against the live geocoder. Four mechanisms: `_pick_best_match` keeps an incoherent fine match over a correct coarse one (the geocoder returned the right colonia and the client discarded it); missing KB entries (Parque España) silently drop anchors; bare streets geocoded one-request-per-site with a dead `MUN="Ciudad de México"` mention, so sibling route context never disambiguates; dirty KB rows (the `zocalo`/Av. Tláhuac entry behind the 11956 mis-pin). Fix order: client coherence rule → MUN remap → sibling batching + outlier filter → KB cleanup. Includes the extraction-side finding that the *"street: colonia"* prose pattern loses its colonia.
- [Extraction leveling: localities/alcaldías end up in `place_name`](docs/todos/location_leveling_extraction.md) — settlements and admin areas extracted as venues stall at coarse precision and poison the venue-creation pipeline; prompt-side fix plus a cached-Google-types re-leveling for the backlog (downstream half in the geocoding repo).
- [Geocode with document context (rescue no-anchor events)](docs/todos/geocode_document_context.md) — ~96 events carry a venue/street but no state/city; use sibling-record locations, source metadata, or full-document NER as geocoder context (group 2). Overlaps grounding Phase 1.
