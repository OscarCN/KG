...
- [Assess: skip the LLM on leaf-location disagreement](docs/todos/skip_llm_on_leaf_disagreement.md) — the soft `ubicacion_fina` signal can be overruled by the LLM (it didn't stop the México↔Belgrado over-merge). Hard-stopping the LLM when leaf ids disagree would, but its safety depends on geocoder leaf accuracy — measure and decide (the geocoder is being improved).
...
- [Retrieval idea: hard date+location, soft name+type](docs/todos/retrieval_name_soft_type.md) — invert the candidate filter: make date overlap + hierarchical geo-compatibility the hard gates, demote exact `event_type` to a soft signal, and add name-similarity retrieval (trigram first, LSH later). Retrieves the cross-type/cross-precision variants of one event (the Zona Fest case) and produces the multi-match candidate set the reconciliation pass acts on.
- [Tiered extraction: essential vs secondary fields](docs/todos/tiered_extraction_essential_fields.md) — tag each schema attribute `importance: essential | secondary`, generate an essential-only prompt (identification + description/context/status) used by default, and extract secondary fields only for important events. Saves extraction tokens; the essential set is exactly what the linker needs. A pre-productionization cost optimization, parallel to the linking work.
- [Persist linked events into kgdb (Step Zero → streaming write/merge)](docs/todos/kgdb_event_persistence.md) — write linked records into the unified kgdb Postgres DB; eventual end-state is a RabbitMQ consumer that writes/merges continuously (events + entities). **Sequenced after** the linking-quality work above — *near-term order:* [multi-venue/multi-street locations](docs/todos/location_level_list_extraction.md) → [soft name/type retrieval + multi-match](docs/todos/retrieval_name_soft_type.md) → [canonical reconciliation](docs/todos/canonical_reconciliation.md) → **then** persistence (persisting today would write the known-fragmented output into the DB). Step Zero is a decoupled batch writer with prerequisites (fix the `entity_locations` identity bug; seed the type catalog).
