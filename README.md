...
- [Deterministic merge slack: no-name level-6/7 branch](docs/todos/deterministic_match_slack.md) — **implemented**: the deterministic gate keys on a shared `level_6_id`/`level_7_id` + day (name-agnostic, precision-aware), replacing the coords-distance branches and fixing a precision-blindness bug. Doc retains the rationale + accepted weaknesses (same-street collisions; coarse under-merge → see the location-list TODO).
- [Review: precision-6/7 records that didn't merge deterministically](docs/todos/didnt_merge_review.md) — case-log findings: the extracted-date guard blocks same-street incident merges (publication-only dates), the LLM over-merges across *different* fine streets (Paseo de México ↔ Belgrado sinkholes), and multi-street projects fragment. Patterns A (leaf publication-date relaxation) and B (`ubicacion_fina` LLM signal) now implemented.
