...
- [Document retrieval strategy (keyword pre-filter → kg doc queue)](docs/todos/document_retrieval_strategy.md) — the **producer** side the streaming listener is missing: a new retriever compiles the active `keywords.xlsx` rows into an ES query (via `elastic_client`) and polls ES, publishing matching documents to the kg doc queue. Decided **global ontology KG** (all news matching the ontology keywords, independent of saved searches). Coarse ES retrieve + precise in-worker `Ontology.match`; `src/PoC/get_entities_data.py` already prototypes keywords→ES for one row.
