...
- [Deterministic merge slack: no-name level-6/7 branch](docs/todos/deterministic_match_slack.md) — replace the coords-distance deterministic branches with a precision-aware "shares `level_6_id`/`level_7_id` + day + type, no name" rule (fixes a precision-blindness bug). Self-contained handoff, approved and ready to implement on the `geo-deterministic-linking` branch.
