...
| `event_date_start` / `event_date_end` | The **asserted event dates for user-facing reads** — the extracted `date_range` verbatim, no slack. `event_date_end` NULL = punctual / no known end (consumers must fall back as a *pair*, never mix one column from each pair). When nothing was extracted, the publication date is stored as **both** start and end. **First-report time stamp**: a date-only start (local midnight — the article asserted the day but no time) of a same-day, non-planned event is promoted to the **earliest source publication timestamp** (`_first_report_stamp`): the first article/post about an already-happened event — social especially — is a tight upper bound on the event time. Guards: never overwrites an extracted time; skipped for planned-like statuses (`planned`/`postponed`/`cancelled`/`delayed`/`suspended` — announcement time ≠ event time), for multi-day ranges, and when the earliest report lands on a later local day or after a same-day asserted end. A punctual `start == end` range promotes both. Flagged in `entities.metadata` as `_event_time_source: "first_report"`; the extracted `date_range` in metadata stays verbatim. Recomputed on every create/merge, so an earlier source arriving later tightens the stamp. | `KgdbWriter._event_dates` |
