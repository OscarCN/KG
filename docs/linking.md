...
**Provenance survives the envelope by rule.** `_normalize_envelope` runs the record through the supertype's schema `Parser`, which drops every field the schema doesn't declare. So it sets aside `date_created` / `news_type` **plus every `_`-prefixed key** (`_source_id`, `_supertype`, `_images`, `_author_geo`, …) before parsing and merges them back after. The convention is load-bearing: `_`-prefix means provenance, never a schema field. Adding a new provenance field therefore needs no change here — an earlier per-field whitelist silently swallowed `_images` (so `_sources[].images` was always `[]` and `entities_documents.doc_images` was written empty for every streamed document) and `_author_geo` (so author-context geocoding never fired).
